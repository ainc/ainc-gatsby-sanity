  ...
  return (
    <Layout>
      <Container>
        <Row>
          <Col>
            <Title className={`${styles.headingText} text-uppercase`}>
              Podcast Episodes
            </Title>
          </Col>
        </Row>
          ...
          <Col>
            {!isFirst && (
              <Link
                ...
                to={`${prevPage == 1 ? "" : `/podcast/${prevPage}`}`}
                rel="prev"
              >
                ← Previous Page
              </Link>
      ...
      <h5 className="text-center mb-5 mt-5">
        <Row className={`g-0 ${styles.pageNumberSingle}`}>
          ...
          <Col className={styles.page}>
            {currentPage}/
            <Link className={`mx-0 ${styles.page}`} to={`/podcast/${numPages}`}>
              {numPages}
            </Link>
          </Col>
          <Col className="d-flex align-content-center justify-content-center">
...
export const podcastInfo = graphql`
  query ($skip: Int!, $limit: Int!) {
    allFeedAnchorPodcast(sort: { isoDate: DESC }, skip: $skip, limit: $limit) {
      totalCount
      nodes {
        title
        pubDate
