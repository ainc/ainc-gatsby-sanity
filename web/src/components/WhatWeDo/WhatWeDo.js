...
const WhatWeDoBox = ( {item} ) => {
    ...
    <Container className="d-flex flex-column justify-content-center align-items-center text-center">
        <div className={`${styles.logoContainer} d-flex flex-column justify-content-center align-items-center text-center`}>
            <a href={item.linkToPage} className="d-flex align-items-center">
                <Image className={`${styles.logo}`} src={item.logo.asset.url} alt={item.title}/>
                {partnerships.includes(item.title) && <Image className={`${styles.asterisk}`} src={redAsterisk} alt="Red asterisk"/>}
            </a>
        </div>
