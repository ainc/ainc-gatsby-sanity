...
const InterviewPage = () => {
    ...
    const month = StarttimeStamp.toLocaleString('en-US', {weekday: 'long',
    month: 'long',
    day: 'numeric',
    year: 'numeric',})
        ...
        <Container>
            <Col lg={{span:12, offset: 0}}  className='' >
                ...
                <Row className='mt-5 my-1'>
                    ...
                    <p className='ms-3' style={{fontSize: "1.5 rem"}}> Please check your inbox at {invitee} and confirm that you can make the scheduled time.</p>
                    </Col>
                ...
                <Row className='justify-content-center'> 
                    {!isTimerComplete ? (
                        <CountdownCircleTimer 
                            ...
                            {({ remainingTime}) => (
                                <div>
                                <h1 style={{ fontSize: 16, marginLeft: "30px", marginTop: "5px", fontWeight: "bold"}}>{`${Math.floor(remainingTime / 60).toString().padStart(2, '0')}:${(remainingTime % 60).toString().padStart(2, '0')}`}</h1>
                                </div>
                            )}
                        </CountdownCircleTimer>
                    ) : (
