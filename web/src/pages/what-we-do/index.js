...
const WhatWeDoPage = ({ data }) => {
  ...
  const entrepreneurship = nodes.filter(
    (node) => node.category === "entrepreneurship",
  );
          ...
          <Row className="">
            <Col md={9} lg={7} className="mt-3 ">
              <div className={`{${styles.heading} pt-5 mt-lg-5`}>
                <Title
                  className={` ${styles.headingTitle} text-uppercase text-white mt-5`}
                >
                  Why awesome inc exists
                </Title>
                <Subtitle
                  className={` ${styles.headingSubtitle} text-white mt-4 mb-4`}
                >
      ...
      <section className={styles.workHeader}>
        <Container className={styles.whatWeDo}>
          <Row>
            ...
            >
              <Title
                className={`${styles.programs} mb-5 text-danger text-center text-uppercase`}
              >
                Our programs
              </Title>
              <p className={`${styles.programsSubheader} text-center mt-3`}>
                Awesome Inc was founded on the basis of the "give-first"
                mentality. Awesome Inc started in 2009 and has grown to be the
                epicenter for anything related to technology or business. We
                strive to make Lexington a better place to live and work through
          ...
          <Row className="justify-content-center">
            <Col
              className={`align-items-md mb-sm-4 d-flex justify-content-md-center justify-content-sm-center`}
              lg={4}
              sm={12}
            >
              <Image
                className={`${styles.headerImg} mx-auto`}
                src={computerStare}
                alt="Computer Stare"
              />
            </Col>
            <Col
              className={`align-items-md-end mb-sm-4 d-flex justify-content-md-center justify-content-sm-center`}
              lg={4}
              md={12}
            >
              <Image
      ...
      <section className="mt-5">
        <Container className={styles.entrepreneurship}>
          ...
          <Row className={`${styles.logoHeader}`}>
            <h2
              className={`text-uppercase text-danger text-center ${styles.headingTitle}`}
            >
              Entrepreneurship
            </h2>
            <hr
              style={{ color: "black", backgroundColor: "black", height: 6 }}
            ...
            {entrepreneurship.map((node, index) => {
              return (
                <React.Fragment key={index}>
                  {index % 3 === 0 && index !== 0 && (
                    <Image
                      className={`${styles.dotDivider} d-flex justify-content-center`}
                      src={dotDivider}
                      alt="Dot Divider"
                    />
                  )}
      ...
      <section className="mt-5">
        <Container className={``}>
          ...
          <Row className="align-items-center justify-content-center mb-5">
            <Col
              xs="auto"
              className="d-inline-flex align-items-center mb-5 mt-3 pt-4"
            >
              <Image
                className={`${styles.asterisk} mt-5`}
                src={redAsterisk}
                alt="Red asterisk"
              />
              <p className="text-center mb-0 ml-2 mt-5">
                Awesome Inc is a contributor of this program, not an owner
              </p>
            </Col>
          </Row>
        </Container>
