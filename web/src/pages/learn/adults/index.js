...
const AdultsPage = ({ data }) => {
  const FTCodingCourses = data.allSanityCourses.edges.filter(
    (course) =>
      course.node.timeRequirement === "full-time" &&
      course.node.courseType === "coding",
  );
  const PTCodingCourses = data.allSanityCourses.edges.filter(
    (course) =>
      course.node.timeRequirement === "part-time" &&
      course.node.courseType === "coding",
  );
  const otherCourses = data.allSanityCourses.edges.filter(
    (course) => course.node.courseType === "other",
  );
        ...
        <Row className={`${styles.box} pt-2 mt-3`}>
          <Col sm="5" className={`my-5 pt-3 all-padding`}>
            <Row className={`justify-content-center`}>
              ...
              <Col md="6">
                <p className="fs-6">
                  <i>
                    Full and Part-Time Courses for Career Switchers,
          ...
          <Col md={6}>
            <StaticImage
              placeholder="blurred"
              quality="90"
              className={`${styles.macImage} img-responsive`}
              src="../../../images/mac-code.png"
      ...
      <Container>
        <Row className="mt-5">
          <Col lg={{ span: 5, offset: 6 }} className="text-center mb-5">
            <p>
              Learning to code is difficult, but so is learning to salsa dance
              or learning to play the guitar. Consistent practice, week after
              week, is how you become a great programmer.
            </p>
          </Col>
        </Row>
        <Row>
          <Col xs="12" md="6">
            <a href="https://youtu.be/mL5h5DqhXjM" target="_blank">
              <StaticImage
                placeholder="blurred"
                src="../../../images/macbook-youtube-video.png"
                alt="macbook youtube"
                quality="90"
              />
            </a>
          ...
          <Col xs="12" md="6" className="d-flex align-items-end mb-5">
            <p>
              <i>
                "Starbucks to Software Developer in 12 Weeks."
                <br />
                Web Developer Bootcamp Student Spotlight: David V.
              </i>
            </p>
          </Col>
        </Row>
